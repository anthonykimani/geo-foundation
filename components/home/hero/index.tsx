...
function HeroSection() {
  ...
  const heroVideoUrl = homePageData?.heroVideoUrl;
    ...
    <section ref={ref} className="relative overflow-hidden">
      {heroVideoUrl ? (
        <div className="relative w-full h-screen overflow-hidden">
          <VideoEmbed url={heroVideoUrl} title="Hero background" background />
            ...
            <div className="flex flex-col max-w-xl gap-6 sm:gap-8">
              ...
              <motion.div {...bottomAnimation} className="mt-4">
                ...
                <div className="flex justify-between text-sm sm:text-base text-white/90 mb-2">
                  <span><AnimatedNumber value={bricksRaised} inView={inView} /> Bricks Raised</span>
                  <span>{targetBricks.toLocaleString()} Targeted Bricks</span>
                </div>
              </motion.div>
              <motion.div {...bottomAnimation} className="flex flex-col md:flex-row gap-8">
                <Link href="/get-involved" onClick={() => trackEvent("hero_join_movement_click")}>
                  <Button className="relative text-sm font-medium rounded-full h-12 p-1 ps-6 pe-14 group transition-all duration-500 hover:ps-14 hover:pe-6 w-fit overflow-hidden">
                    <span className="relative z-10 transition-all duration-500">Join the Movement</span>
        ...
        >
            ...
            {displayImages.map((image, index) => {
              ...
              <CarouselItem key={index} className="relative w-full h-screen">
                <div className="absolute inset-0">
                  {imageUrl && (
                  <Image src={imageUrl} alt={image.alt} fill className="object-cover object-top" priority={index === 0} />
                  )}
                ...
                <div className="relative h-full container pt-28 sm:pt-40 md:pt-52 2xl:pb-20 pb-10 flex items-center">
                  <div className="flex flex-col max-w-xl gap-6 sm:gap-8">
                    <div className="relative flex flex-col text-left items-start sm:gap-6 gap-4">
                      <h1><TextGenerateEffect words="Make an Impact" className="text-white whitespace-nowrap" /></h1>
                      <h1><TextGenerateEffect words="Brick by Brick" delay={0.8} className="font-instrument-serif italic tracking-tight text-teal-brand" /></h1>
          ...
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex gap-2 z-20">
            {displayImages.map((_, index) => (
              <button
                key={index}
                onClick={() => { api?.scrollTo(index); trackEvent("hero_slide_nav_click", { slide: index }); }}
